...
```
NOTE: b is set to 0.3 which means that we expect the effect size of register on articulation to be between -0.6 and + 0.6.
Effect sizes in social psychology is typically +- 0.2, so this is rather conservative allowing for different values but also not crazy values
Sigma is set as in the first video, so is intercept. 
...
Answer: When using IDS compared to ADS, articulation drops with 0.37 on average. 
This means that vowel spaces drop in IDS
