...
Answer: Multi-level sigma: 0.83 vs simple regression sigma: 0.98. Now that we model individual differences and participants are allowed to vary differently and not just in the same way all of them, the model makes fewer mistakes and deviation from the estimated values is smaller leading to a lower sigma. 
...
```{r}
plot_df <- tibble(
  # Get list of two-letter ID 
  Subject = rownames(coef(Articulation_m3)[["Subject"]][,,"Intercept"]), 
  # Get the ADS estimate for each participant, i.e. sort of intercept
  ADS = coef(Articulation_m3)[["Subject"]][,,"Intercept"][,1], 
  # Get the effect using IDS compared to ADS for each participant (i.e., ADS + IDS effect)
  IDS = ADS + coef(Articulation_m3)[["Subject"]][,,"RegisterIDS"][,1],
  Type = "partial pooling") %>% 
  # It is now a df with the four columns defined above --> columns ADS, IDS and type for each ID
  pivot_longer(ADS:IDS) %>% # This one collapses the two columns ADS and IDS
  dplyr::rename(
  Register = name,
  ArticulationS = value
)
df <- d[, c("Subject", "Register", "ArticulationS")] %>% # Take original data
  mutate(Type = "no pooling") # Add column saying "no pooling"

# Take the df with original data
pool_df <- df[,c("Subject", "Register")] %>%
  mutate(
    # Make column indicating the mean of each of the two groups, ADS and IDS
    ArticulationS = ifelse(Register=="ADS", mean(df$ArticulationS[df$Register=="ADS"]), mean(df$ArticulationS[df$Register=="IDS"])),
    # Add column saying "total pooling"
    Type = "total pooling"
  )
# df, plot_df and pool_df are equally long now 

plot_df <- rbind(plot_df,df) 
plot_df <- rbind(plot_df,pool_df) 

# Turn register column from character (ADS, IDS) to numeric (1,2)
plot_df <- plot_df %>%
...
## Original data
df_no <- NULL
# Creates a tibble with the ADS estimate for each participant and the effect of using IDS instead
...
# Only fixed effect, no random effects
...
Answer: There are not many differences between the two 
__________________________________________________________________________________________________________________________________________
